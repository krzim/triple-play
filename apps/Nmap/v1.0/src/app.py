...
class Nmap(Appbase):
    ...
    def run_scan(self, target, options, output_filename=None):
    ...
    def scan_results_as_json(self, nmap_out, is_file=False, output_filename=None):
    ...
    def graph_from_results(self, port_scan_xml_filename, traceroute_xml_filename, openvas_json_filename, output_filename):
    ...
    def ports_and_hosts_from_json(self, nmap_json, is_file=False):
        try:
            if is_file:
    ...
    def add_host_to_whitelist(self, host, whitelist):
        return whitelist.append(host)


    def add_host_to_blacklist(self, host, blacklist):
        return blacklist.append(host)
    ...
    def run_scan_check_whitelist(self, target, options, whitelist):
        nmap_proc = NmapProcess(targets=target, options=options)
        nmap_proc.run()
    ...
    def run_scan_check_blacklist(self, target, options, blacklist):
        nmap_proc = NmapProcess(targets=target, options=options)
        nmap_proc.run()
